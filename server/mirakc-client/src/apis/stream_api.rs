...
pub async fn check_channel_stream(configuration: &configuration::Configuration, r#type: models::ChannelType, channel: &str, x_mirakurun_priority: Option<i32>, decode: Option<bool>, pre_filters: Option<Vec<String>>, post_filters: Option<Vec<String>>) -> Result<(), Error<CheckChannelStreamError>> {
    ...
    let local_var_uri_str = format!("{}/channels/{type}/{channel}/stream", local_var_configuration.base_path, type=r#type.to_string(), channel=crate::apis::urlencode(channel));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CheckChannelStreamError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn check_service_stream(configuration: &configuration::Configuration, id: i64, x_mirakurun_priority: Option<i32>, decode: Option<bool>, pre_filters: Option<Vec<String>>, post_filters: Option<Vec<String>>) -> Result<(), Error<CheckServiceStreamError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CheckServiceStreamError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
