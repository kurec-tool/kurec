...
pub async fn check_version(configuration: &configuration::Configuration, ) -> Result<models::Version, Error<CheckVersionError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CheckVersionError> = serde_json::from_str(&content).ok();
